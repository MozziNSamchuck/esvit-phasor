...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.shift_size > 0:
            ...
            if (H, W) in self.attn_mask_dict:
                attn_mask = self.attn_mask_dict[(H, W)]
            else:
                self.attn_mask_dict[(H, W)] = self.create_attn_mask(H, W).to(x.device)
                attn_mask = self.attn_mask_dict[(H, W)]
...
class BasicLayer(nn.Module):
    ...
    def forward_with_features(self, x, H, W):
        fea = []
        for blk in self.blocks:
            x, _ = blk(x, H, W)
            fea.append(x)
    ...
    def forward_with_attention(self, x, H, W):
        attns = []
        for blk in self.blocks:
            x, attn = blk(x, H, W)
            attns.append(attn)
...
class SwinTransformer(nn.Module):
    ...
    def forward_selfattention(self, x, n=1):
        ...
        if n==1:
            return self.forward_last_selfattention(x, H, W)
        else:
            return self.forward_all_selfattention(x, H, W)

    def forward_last_selfattention(self, x, H, W):

        for i, layer in enumerate(self.layers):
            if i < len(self.layers) - 1:
                x, H, W = layer(x, H, W)
            else:
                x, attns, H, W = layer.forward_with_attention(x, H, W)
                return attns[-1]

    def forward_all_selfattention(self, x, H, W):
        attn_out = []
    ...
    def forward_return_n_last_blocks(self, x, n=1, return_patch_avgpool=False, depth=[]):
        ...
        s = 0
        for i, layer in enumerate(self.layers):
            x, fea, H, W = layer.forward_with_features(x, H, W)
...
def get_cls_model(config, is_teacher=False, use_dense_prediction=False, **kwargs):
    ...
    swin = SwinTransformer(
        ...
        in_chans=swin_spec.get('IN_CHANS', 3),
