...
def get_args_parser():
    ...
    parser.add_argument('--frame_stride', default=50, type=int,
        help='Phasor frame stride for sampling. 기본: 50 (1000fps 기준 50ms 간격)')
    parser.add_argument('--start_stride', default=5, type=int,
        help='Phasor start frame stride. 기본: 5')
    
    parser.add_argument('--stats_path', default='', type=str,
        help='정규화 통계값 JSON 경로. 비어있으면 build.py 기본 경로(stats.json) 사용.')
    ...
    parser.add_argument('--grad_accum_steps', default=1, type=int,
        help='Number of gradient accumulation steps before optimizer update. '
             'Effective batch = batch_size_per_gpu * num_gpus * grad_accum_steps.')
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for local_it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            # gradient accumulation: loss를 step 수로 나눠서 스케일 유지
        ...
        # grad_accum_steps마다 optimizer step + EMA 업데이트
        is_update_step = (local_it + 1) % args.grad_accum_steps == 0
