...
def build_dataloader(args, is_train=True):
    ...
    if 'bolus_sf' in args.dataset:
        from .phasor_dataset import DataAugmentationPhasor
        from .bolus_sf_dataset import BolusSFDirectDataset
        import json as _json

        transform = DataAugmentationPhasor(
        ...
        stats = None
        _custom = getattr(args, 'stats_path', '')
        if _custom and os.path.exists(_custom):
            stats_path = _custom
        else:
            stats_path = os.path.normpath(os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
                '..', 'datasets_sf', 'stats_sf.json'))
        if os.path.exists(stats_path):
            with open(stats_path) as f:
                stats = _json.load(f)
            print(f"정규화 통계값 로드: {stats_path}")
        else:
            print(f"[경고] stats_sf.json 없음: {stats_path}")

        csv_path = os.path.normpath(os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            '..', 'datasets_sf', 'sample_index_table.csv'))

        _n_frames = 3 if args.dataset == 'bolus_sf3' else 1
        dataset = BolusSFDirectDataset(
            csv_path=csv_path,
            iq_dir=args.data_path,
            split='pretrain',
            transform=transform,
            stats=stats,
            cache=True,
        ...
        print(f"Data loaded: there are {len(dataset):,} bolus_sf samples.")
        return data_loader

    if 'bolus_v5' in args.dataset:
        from .phasor_dataset import DataAugmentationPhasor
        from .bolus_v5_dataset import BolusMATDirectDataset
        import json as _json
        ...
        csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                '..', 'datasets_v5', 'sample_index_table.csv')
        csv_path = os.path.normpath(csv_path)

        dataset = BolusMATDirectDataset(
            csv_path=csv_path,
            iq_dir=args.data_path,
            split='pretrain',
            transform=transform,
        ...
        print(f"Data loaded: there are {len(dataset)} bolus_v5 samples.")
        return data_loader

    if 'bolus_v4' in args.dataset:
        from .phasor_dataset import DataAugmentationPhasor
        from .bolus_mat_dataset import BolusMATPretrainDataset
        import json as _json
        ...
        stats = None
        stats_path = os.path.join(os.path.dirname(args.data_path), 'stats.json')
        if os.path.exists(stats_path):
            with open(stats_path) as f:
                stats = _json.load(f)
            print(f"정규화 통계값 로드: {stats_path}")
        ...
        dataset = BolusMATPretrainDataset(
            args.data_path,
            transform=transform,
            stats=stats,
            n_frames=10,
        ...
        print(f"Data loaded: there are {len(dataset)} bolus_v4 samples.")
        return data_loader

    if 'phasor' in args.dataset:
        from .phasor_dataset import DataAugmentationPhasor
        from .block_dataset import BlockPretrainDataset
        ...
        block_dirs = args.data_path.split(',')

        # stats.json: 첫 번째 블록 디렉토리의 상위 상위 폴더에서 탐색
        stats = None
        stats_path = os.path.join(os.path.dirname(os.path.dirname(block_dirs[0])), 'stats.json')
        if os.path.exists(stats_path):
            with open(stats_path) as f:
                stats = json.load(f)
            print(f"정규화 통계값 로드: {stats_path}")
        ...
        dataset = BlockPretrainDataset(
            block_dirs,
            transform=transform,
            stats=stats,
            frame_stride=args.frame_stride,
            start_step=args.start_stride,
        )
        ...
        print(f"Data loaded: there are {len(dataset)} phasor samples.")
        return data_loader
