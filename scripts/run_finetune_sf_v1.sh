#!/bin/bash
# Fine-tuning sf_v1: single-frame backbone + ft+pretrain GT
#   backbone:   pretrain_sf_v1 (2ch, mag+phase)
#   SegHead:    joint 3-class (noise/tissue/bubble)
#   데이터:     ft_train + pretrain (124,800개)
#   GT:         GT_v1/gt (per-frame, ignore→255)
#   비교대상:   finetune_v44 (20-frame, 3cls, ft+pretrain)

set -e
cd "$(dirname "$0")/.." 

OUTPUT_DIR="OUTPUT/finetune_sf_v1"
mkdir -p $OUTPUT_DIR
